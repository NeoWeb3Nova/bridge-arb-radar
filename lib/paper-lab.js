...
const VERIFICATION_MS = 8 * 60 * 1000;
...
function snapshot() {
  ...
  return { ok: true, enabled: s.enabled, busy, discoveryBusy, mode: s.mode, intervalSeconds: 30,
    defaults: DEFAULTS, requestsPaused: traffic.isPaused(), verificationIntervalSeconds: VERIFICATION_MS / 1000,
    nextVerificationAt: s.enabled ? s.nextVerificationAt || null : null,
    providerBackoffUntil: Math.max(s.providerBackoffUntil || 0, traffic.cooldownUntil('li.quest')) > Date.now() ? Math.max(s.providerBackoffUntil || 0, traffic.cooldownUntil('li.quest')) : null, candidates: candidates(), reports: s.reports.slice(0, 100), samples: s.reports.length,
...
async function discoveryTick() {
  const s = load();
  if (!s.enabled || discoveryBusy || traffic.isPaused()) return;
  discoveryBusy = true;
...
async function check(params, internal = false) {
  ...
  if (internal && !load().enabled) return { ok: false, error: '自动监控已停止' };
  if (traffic.quoteBudget().remaining < 4) return { ok: false, error: '本机报价预算不足以完成四段验证，等待额度恢复' };
  ...
  busy = true;
  const s = load(), startedGeneration = generation;
  const shouldContinue = () => !traffic.isPaused() && (!internal || (s.enabled && generation === startedGeneration));
  ...
  try {
    ...
    const report = await validateCycle(params, settings, { shouldContinue, onStage: stage => { if (s.current) s.current.stage = stage; } });
    ...
    if (!shouldContinue()) {
      report.review = { state: 'MONITOR_STOPPED', executable: false, blockers: ['监控或外部请求已暂停，未继续执行后续复核'] };
    } else if (report.status === 'POSITIVE_INDICATION') {
      ...
      if (report.repeatCount < 2 && s.enabled && shouldContinue()) {
...
async function tick() {
  const s = load();
  if (!s.enabled || busy || traffic.isPaused()) return;
  ...
  const i = s.queue.findIndex(q => q.dueAt <= Date.now() && (q.source === 'automatic_recheck' || Date.now() >= (s.nextVerificationAt || 0)));
  if (i < 0 || Math.max(s.providerBackoffUntil || 0, traffic.cooldownUntil('li.quest')) > Date.now() || traffic.quoteBudget().remaining < 4) return;
  const [candidate] = s.queue.splice(i, 1);
  if (candidate.source !== 'automatic_recheck') s.nextVerificationAt = Date.now() + VERIFICATION_MS;
  await check(candidate, true);
}
function configure(enabled) {
  const s = load();
  if (s.enabled !== (enabled === true)) generation++;
