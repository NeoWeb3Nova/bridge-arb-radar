...
const ENDPOINT = 'https://explorer-api.hyperlane.xyz/graphql';
...
function normalizeAddress(v) {
  ...
  const h = hex(v);
  if (!h) return null;
  // 若为 66 位且高 12 字节全为 0，则是 32 字节对齐的 EVM 地址
  if (/^0x0{24}[a-fA-F0-9]{40}$/.test(h)) {
    return ('0x' + h.slice(26)).toLowerCase();
  }
...
class HyperlaneAdapter extends BaseBridgeAdapter {
  ...
  normalize(m) {
    const rawTs = m.send_occurred_at;
    const isoTs = rawTs
      ? new Date(rawTs.includes('T') || rawTs.endsWith('Z') ? rawTs : rawTs.replace(' ', 'T') + 'Z').toISOString()
      : null;
    return {
      source: 'hyperlane',
      ...
      receiver: normalizeAddress(m.recipient),
      tokenAddress: null,
      tokenSymbol: null,
      amount: null,
      amountUsd: null,
