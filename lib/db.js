...
function loadWallets() {
  ...
  const out = {};
  for (const r of rows) { const w = JSON.parse(r.data); out[addressKey(w.address)] = w; }
  return out;
}
...
function loadTokens() {
  ...
  const out = {};
  for (const r of rows) { const t = JSON.parse(r.data); out[`${t.chain}:${addressKey(t.address)}`] = t; }
  return out;
}
...
function walletRow(w) {
  return [addressKey(w.address), w.score ?? 0, w.grade ?? null, w.lastSeen ?? null, j(w)];
}
function tokenRow(t) {
  return [`${t.chain}:${addressKey(t.address)}`, t.chain, addressKey(t.address), t.symbol ?? null, j(t)];
}
function oppRow(o) {
...
function persistDelta(delta) {
  ...
  try {
    ...
    for (const addr of delta.walletDeletes || []) s.delWallet.run(addressKey(addr));
...
function queryTransfers({ q, source, chain, unknown, minUsd, hours, limit = 200, offset = 0 } = {}) {
  ...
  if (unknown === '1' || unknown === true) {
    const list = Array.from(COMMON);
    where.push(`token_symbol IS NOT NULL AND token_symbol != '' AND UPPER(token_symbol) NOT IN (${list.map(() => '?').join(',')})`);
    args.push(...list);
  }
  if (q) {
...
function queryWallets({ q, grade, sort = "score", starred, hideContracts, limit = 200, offset = 0 } = {}) {
  const where = [];
  const args = [];
  if (grade) {
    where.push("grade = ?");
  ...
  if (starred === "1" || starred === true) {
    where.push("json_extract(data, '$.starred') = 1");
  } else {
    where.push("(json_extract(data, '$.ignored') IS NULL OR json_extract(data, '$.ignored') = 0)");
  }
  if (hideContracts === "1" || hideContracts === true) {
    where.push("((json_extract(data, '$.likelyContract') IS NULL OR json_extract(data, '$.likelyContract') = 0) AND (json_extract(data, '$.passiveAccount') IS NULL OR json_extract(data, '$.passiveAccount') = 0))");
  }
  if (q) {
    const s = "%" + String(q).toLowerCase() + "%";
  ...
  else if (sort === "bridges") orderBy = "CAST(json_extract(data, '$.bridgeCount') AS INTEGER) DESC";
  else if (sort === "cycles") orderBy = "CAST(json_extract(data, '$.capitalCycles') AS INTEGER) DESC";
  else if (sort === "roundtrips") orderBy = "CAST(json_extract(data, '$.roundtrips') AS INTEGER) DESC";

  const w = where.length ? "WHERE " + where.join(" AND ") : "";
  const d = open();
  const total = d.prepare(`SELECT COUNT(*) c FROM wallets ${w}`).get(...args).c;
  const rows = d.prepare(`SELECT data FROM wallets ${w} ORDER BY ${orderBy} LIMIT ? OFFSET ?`).all(...args, Number(limit), Number(offset));
  return { total, items: rows.map((r) => JSON.parse(r.data)) };
}
  ...
  const rowCount = (() => {
    try {
      ...
      return {
        ...
        opportunities: d.prepare('SELECT COUNT(*) c FROM opportunities').get().c,
      };
    } catch { return null; }
  })();
  return {
...
/**
 * 重置数据库：安全清空业务表并重置元数据，可选择保留配置与 API 密钥。
 * 执行前自动对当前主库做一份 pre-reset 应急快照备份，防止意外误删。
 */
function resetDatabase({ keepSettings = true, defaultSettings = null } = {}) {
  const d = open();
  const s = S();
  ...
  try {
    ...
    const dest = path.join(BACKUP_DIR, `radar-pre-reset-${stamp}.db`);
    backup(dest);
    backupFile = dest;
  } catch (err) {
    console.warn('[db] 重置前自动备份失败（非致命）：', err.message);
  }

  // 2. 提取当前配置（若需保留）
  let savedSettings = null;
  if (keepSettings) {
    savedSettings = loadMeta('settings') || defaultSettings;
  ...
  try {
    ...
    // 重新填充核心 meta
    s.setMeta.run('version', '1');
    if (savedSettings) {
      s.setMeta.run('settings', j(savedSettings));
    }
    s.setMeta.run('stats', j({ transfersSeen: 0, scans: 0, lastScanAt: null }));
    s.setMeta.run('decisions', j({}));
