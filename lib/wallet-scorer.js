...
function detectCapitalCycles(w) {
  ...
  for (const back of sorted) {
    ...
    const match = sorted.find((f) => {
      ...
      if (!isBackInitiated) {
        if (!back.peer || !f.peer || back.peer.toLowerCase() !== f.peer.toLowerCase()) {
          return false;
        }
      }
      return true;
...
function scoreSingleWallet(w, maxBridges = 0, now = Date.now()) {
  ...
  const likelyContract = (isHeavyVolume && (w.capitalCycles || 0) === 0 && roundtrips === 0) || Boolean(w.likelyContract);
  ...
  // 异常惩罚：
  // 1. 被动接收/归集地址封顶在 15 分以内 (归入 D 级)
  // 2. 疑似桥路由/协议合约封顶在 20 分以内 (归入 D 级)
  ...
  w.autoTags = [
    ...
    !passiveAccount && (w.capitalCycles || 0) >= 2 ? '职业套利者' : null,
    !passiveAccount && (w.capitalCycles || 0) >= 1 ? '资金闭环' : null,
    !passiveAccount && roundtrips >= 2 ? '同币往返×2' : null,
    !passiveAccount && roundtrips >= 1 ? '同币往返' : null,
    (w.maxUsd || 0) >= 100000 ? '大额' : null,
    chainCount >= 3 ? '多链活跃' : null,
    exotic.length >= 3 ? '偏好冷门币' : null,
    (sentCount >= 20 || totalBridges >= 30) ? '高频桥用户' : null,
