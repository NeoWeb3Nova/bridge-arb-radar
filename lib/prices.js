...
function getCache(address) {
  const e = quoteCache.get(cacheKey(address));
  if (!e) return null;
  if (Date.now() - e.ts > CACHE_TTL_MS) { quoteCache.delete(cacheKey(address)); return null; }
  return e.pairs;
}
function setCache(address, pairs) {
  quoteCache.set(cacheKey(address), { ts: Date.now(), pairs });
}
