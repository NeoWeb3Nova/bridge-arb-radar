...
// 内存缓存：同一轮扫描里同一地址会被多个候选反复查询；TTL 设 30 秒，仍只用于发现而非成交验证。
const CACHE_TTL_MS = 30000;
const PAIR_CACHE_TTL_MS = 10000; // pair 展示行情缓存 10s
...
function bestPair(pairs, dsChain, tokenAddress) {
  ...
  if (tokenAddress) {
    const target = addressKey(tokenAddress);
    const baseMatches = usable.filter((p) => addressKey(p.baseToken?.address) === target);
    if (baseMatches.length > 0) {
      usable = baseMatches;
    } else {
      const quoteMatches = usable.filter((p) => addressKey(p.quoteToken?.address) === target);
      usable = quoteMatches;
    }
  }
...
function summary(pair, chainKey, tokenAddress) {
  ...
  if (tokenAddress) {
    const target = addressKey(tokenAddress);
    const isBase = addressKey(pair.baseToken?.address) === target;
    const isQuote = addressKey(pair.quoteToken?.address) === target;
...
/** 精确指定池地址的实时行情查询 */
async function quotePair(chainKey, pairAddress, settings, tokenAddress, force = false) {
  const c = chains.get(chainKey);
  if (!c || !c.ds || !pairAddress) return null;
  const pKey = `${c.ds}:${addressKey(pairAddress)}:${addressKey(tokenAddress)}`;
  const cached = pairCache.get(pKey);
  if (!force && cached && (Date.now() - cached.ts < PAIR_CACHE_TTL_MS)) {
...
async function quote(chainKey, tokenAddress, settings, pairAddress, force = false) {
  const c = chains.get(chainKey);
  if (!c || !c.ds) return null;

  // 1. 若提供了 pairAddress，优先查目标池（确保 live 报价与套利发现池严格一致）
  if (pairAddress) {
    const pairQuote = await quotePair(chainKey, pairAddress, settings, tokenAddress, force).catch(() => null);
    return pairQuote && pairQuote.priceUsd > 0 ? pairQuote : null;
  }

  // 2. 兜底按 tokenAddress 查询
  if (!tokenAddress) return null;
  let pairs = !force && getCache(tokenAddress);
  if (!pairs) {
    const res = await request(API + tokenAddress, { settings, timeout: 20000 });
    if (!res.ok || !Array.isArray(res.json?.pairs)) return null;
    pairs = res.json.pairs; setCache(tokenAddress, pairs);
  }
  return summary(bestPair(pairs, c.ds, tokenAddress), chainKey, tokenAddress);
}
...
async function multiChainQuotesBatch(items, settings) {
  ...
  for (const it of items) {
    const addr = addressKey(it.address);
    if (!addr) continue;
  ...
  async function fetchBatch(batch, bi, attempt = 1) {
    ...
    if (res.ok) {
      ...
      for (const addr of batch) {
        const relevant = pairs.filter((p) => addressKey(p.baseToken?.address) === addr || addressKey(p.quoteToken?.address) === addr);
    ...
    if (res.status === 429 || res.code) return; // Do not amplify backoff/pause into single-address retries.
