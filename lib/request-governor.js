...
const QUOTE_LIMIT = 60; // Local conservative budget, not a claim about account entitlement.
function createGovernor({ file, now = Date.now, transport, maxActive = 4, maxQueue = 100, maxWaitMs = 15000, quoteLimit = QUOTE_LIMIT, windowMs = WINDOW_MS, policy } = {}) {
  ...
  function quoteBudget() {
    ...
    return {limit:quoteLimit,used:quoteTimes.length,remaining:Math.max(0,quoteLimit-quoteTimes.length),windowMs,resetAt:quoteTimes.length ? quoteTimes[0]+windowMs : null};
  }
...
module.exports={createGovernor,WINDOW_MS,QUOTE_LIMIT};
