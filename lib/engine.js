...
/** 判断一个地址值是否值得纳入钱包库（排除已知的公共桥路由/协议合约） */
function walletWorthy(a) {
  if (!isEvmAddr(a) && !isSolAddr(a)) return false;
  if (walletScorer.KNOWN_ROUTER_CONTRACTS?.has(addressKey(a))) return false;
  return true;
}
...
async function runScan(opts = {}) {
  ...
  // The server/API broadcasts once after releasing the shared scanning flag.
  return report;
}
