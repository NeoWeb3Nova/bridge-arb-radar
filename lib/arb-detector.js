...
class ArbDetector {
  ...
  static evaluateBestOpportunity({ symbol, quotes, tokenKey, maxHeuristicSpreadPct = 25, minLiquidityUsd = 500 }) {
    ...
    // 1. Bridged 包装/过渡代币与 Native 原生代币跨链混淆检测 (如 zkSync Bridged USDC.e vs Solana Native USDC)
    const buySym = String(low.baseToken || low.symbol || symbol || '').toUpperCase();
    const sellSym = String(high.baseToken || high.symbol || symbol || '').toUpperCase();
    const buyRawName = String(low.baseTokenName || '').toLowerCase();
    const sellRawName = String(high.baseTokenName || '').toLowerCase();
    const isBridged1 = /\.(e|b|m)$/i.test(buySym) || /\b(bridged)\b/i.test(buyRawName);
    const isBridged2 = /\.(e|b|m)$/i.test(sellSym) || /\b(bridged)\b/i.test(sellRawName);
    const isBridgedMismatch = (isBridged1 !== isBridged2) && (low.chain !== high.chain);

    // 2. 稳定币脱锚与异常价差熔断 (如 USDC 单价 $0.7071，真实跨链稳定币正常套利价差通常在 0.1% ~ 1.5%)
    const STABLE_SET = new Set([
      'USDT', 'USDC', 'USDG', 'PYUSD', 'FDUSD', 'RLUSD', 'USDS', 'DAI',
      'USDC.E', 'USDCE', 'USDT.E', 'USDT.B', 'BUSD', 'TUSD', 'FRAX', 'LUSD'
    ]);
    const isStableToken = STABLE_SET.has(String(symbol || '').toUpperCase()) ||
                          STABLE_SET.has(buySym) || STABLE_SET.has(sellSym);
    const isStableDepegged = isStableToken && (
      low.priceUsd < 0.92 || low.priceUsd > 1.08 ||
      high.priceUsd < 0.92 || high.priceUsd > 1.08 ||
      spreadPct > 5
    );

    // 3. 交易池 24h 成交量与换手极度低迷检测 (过滤无交易的僵尸池账面虚假价差)
    const buyVol = Number(low.volume24h);
    const sellVol = Number(high.volume24h);
    const hasVol = Number.isFinite(buyVol) && Number.isFinite(sellVol);
    const minVol = hasVol ? Math.min(buyVol, sellVol) : (Number.isFinite(buyVol) ? buyVol : Number.isFinite(sellVol) ? sellVol : Infinity);
    const isZombiePool = (minVol < 3000 && spreadPct > 15) || (minVol < 1000 && spreadPct > 5);

    const isSymbolCollision = isExtremeSpread || nameMismatch || isBridgedMismatch || isStableDepegged || isZombiePool;
    const collisionRisk = isSymbolCollision;
    let collisionReason = null;
    if (isStableDepegged) {
      collisionReason = `稳定币价格严重脱锚/失衡 (买入 $${low.priceUsd.toFixed(4)} / 卖出 $${high.priceUsd.toFixed(4)} 偏离 $1.00 锚定或价差 +${spreadPct.toFixed(1)}% 过大)，系废弃死池或坏账脱锚资产`;
    } else if (isBridgedMismatch) {
      collisionReason = `跨链资产版本不兼容：Bridged 过渡代币（「${low.baseTokenName || buySym}」）与 Native 原生代币（「${high.baseTokenName || sellSym}」）无跨链直接承兑通道`;
    } else if (isZombiePool) {
      collisionReason = `交易池 24h 成交量极低 ($${Math.round(minVol)})，流动性僵死，存在虚假账面价差`;
  ...
  static calculateOpportunityScore(opp) {
    ...
    if (opp.isSymbolCollision || opp.collisionRisk) {
      ...
      scoreComment = '优先研究 · 展示价差与流动性较高';
    } else if (finalScore >= 70) {
      scoreComment = '待验证 · 展示流动性良好';
