...
class ArbDetector {
  ...
  static calculateOpportunityScore(opp) {
    ...
    if (finalScore >= 85) scoreComment = '极佳机会 · 深度与净利俱佳';
    else if (finalScore >= 70) scoreComment = '优质机会 · 流动性良好';
    else if (finalScore >= 50) scoreComment = '可行 · 容量有限需控单';
    else if (opp.poolSkewed || (opp.sellQuoteReserveUsd !== undefined && opp.sellQuoteReserveUsd < 500)) scoreComment = '高危 · 卖出池真实现金枯竭';
    else if (finalScore >= 25) scoreComment = '次优 · 利润受摩擦损耗';
    else scoreComment = '高风险 · 疑假币或深度过浅';
