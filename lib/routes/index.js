...
function createApiRoutes(context) {
  ...
  api['/api/opportunity/live'] = async (ctx) => {
    ...
    // 1. 并发获取两端 DEX 实时现货报价与实时跨链通道费用
    const [buyQuote, sellQuote, liveBridge] = await Promise.all([
    ...
    if (buyQuote && sellQuote && buyQuote.priceUsd > 0 && sellQuote.priceUsd > 0) {
      const liveBuyPrice = buyQuote.priceUsd;
      const liveSellPrice = sellQuote.priceUsd;
      const liveSpreadPct = Number((((liveSellPrice - liveBuyPrice) / liveBuyPrice) * 100).toFixed(3));
