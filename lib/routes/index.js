...
function createApiRoutes(context) {
  ...
  api['/api/transfers'] = async (ctx) => {
    ...
    const res = db.queryTransfers({ q, source, chain, unknown, minUsd, hours, limit, offset });
    if (unknown === '1') {
      res.items = res.items.filter((t) => t.tokenSymbol && !engine.COMMON.has(String(t.tokenSymbol).toUpperCase()));
  ...
  api['/api/wallets'] = async (ctx) => {
    ...
    const res = db.queryWallets({ q, grade, sort, starred, limit, hideContracts });
    return { ok: true, total: res.total, items: res.items };
  };
