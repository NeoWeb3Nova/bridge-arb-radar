...
function createApiRoutes(context) {
  ...
  api['/api/opportunity/live'] = async (ctx) => {
    ...
    const [buyQuote, sellQuote] = await Promise.all([
      buyAddress ? prices.quote(buyChain, buyAddress, settings, buyPairAddress, force === true || force === 'true') : null,
      sellAddress ? prices.quote(sellChain, sellAddress, settings, sellPairAddress, force === true || force === 'true') : null,
    ]);
  ...
  api['/api/settings'] = async (ctx) => {
    ...
    if (req.method === 'POST') {
      ...
      if (patch.notifications) {
        ...
        const curr = d.settings.notifications || base;
