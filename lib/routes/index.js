...
function createApiRoutes(context) {
  ...
  api['/api/opportunity/live'] = async (ctx) => {
    ...
    // 步骤 2：在取得实时代币价格与搬运数量后，再进行链上跨链通道询价（传递实时代币单价与真实搬运数量）
    ...
    if (liveBuyPrice > 0 && liveSellPrice > 0) {
      ...
      const totalCostUsd = liveBridge?.totalFeeUsd != null ? liveBridge.totalFeeUsd : 4.5;
      ...
      if (liveSpreadPct <= 0) {
        ...
        status = 'NARROWED';
        statusMessage = `价差收窄 (${liveSpreadPct}%)：当前毛利 $${grossProfitUsd} 不足以覆盖跨链费用 ($${totalCostUsd})`;
        ...
        status = 'ACTIVE';
        statusMessage = `机会依然成立：实时价差 +${liveSpreadPct}%，预计净利 +$${netProfitUsd} (${netYieldPct}%)`;
      }
    }
