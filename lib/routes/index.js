...
function createApiRoutes(context) {
  ...
  api['/api/opportunity/live'] = async (ctx) => {
    ...
    const [buyQuote, sellQuote] = await Promise.all([
      buyAddress ? prices.quote(buyChain, buyAddress, settings, buyPairAddress) : null,
      sellAddress ? prices.quote(sellChain, sellAddress, settings, sellPairAddress) : null,
    ]);
    ...
    const liveBuyPrice = buyQuote?.priceUsd > 0 ? buyQuote.priceUsd : (Number(snapshotBuyPrice) || 0);
    const liveSellPrice = sellQuote?.priceUsd > 0 ? sellQuote.priceUsd : (Number(snapshotSellPrice) || 0);
    ...
    if (liveBuyPrice > 0 && liveSellPrice > 0) {
      ...
      if (isTrapPool) {
        ...
        status = 'ACTIVE';
        statusMessage = `机会依然成立：实时价差 +${liveSpreadPct}%，扣除池手续费与跨链成本后净利 +$${netProfitUsd} (${netYieldPct}%)`;
      }
    }
