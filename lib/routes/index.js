...
const depegDetector = require('../depeg-detector');
...
function createApiRoutes(context) {
  ...
  const api = {};
  const lab = require('../paper-lab');
  const traffic = require('../net').governor;

  api['/api/depeg/status'] = async () => depegDetector.checkStatus();
  api['/api/depeg/refresh'] = async () => depegDetector.checkStatus({ force: true });
  api['/api/activity'] = async ctx => {
    if (ctx.req.method === 'POST') {
      if (typeof ctx.body.paused !== 'boolean') return { ok: false, error: 'paused 必须为布尔值' };
      traffic.setPaused(ctx.body.paused);
    }
    const monitoring = lab.snapshot();
    return { ok: true, network: traffic.snapshot(), scan: { enabled: store.settings().scan?.autoEnabled === true, busy: isScanning(), nextAt: getNextScanAtMs() },
      monitoring: { enabled: monitoring.enabled, busy: monitoring.busy, discoveryBusy: monitoring.discoveryBusy, nextVerificationAt: monitoring.nextVerificationAt, verificationIntervalSeconds: monitoring.verificationIntervalSeconds } };
  };
  api['/api/lab/state'] = async () => lab.snapshot();
  api['/api/lab/check'] = async ctx => ctx.req.method === 'POST' ? lab.check(ctx.body) : { ok: false, error: 'POST required' };
  api['/api/lab/config'] = async ctx => ctx.req.method === 'POST' ? lab.configure(ctx.body.enabled) : { ok: false, error: 'POST required' };
  ...
  api['/api/state'] = async () => {
    ...
    const allDecisions = Object.values(d.decisions || {});
    const isActivelyMarked = (r) => !!(r && (r.status !== 'todo' || (r.note && r.note.trim()) || (r.journal && r.journal.length > 0) || (r.realizedPnlUsd && r.realizedPnlUsd !== 0)));
    const markedDecisions = allDecisions.filter(isActivelyMarked);
    const decisionLogs = allDecisions.reduce((sum, r) => sum + (r.journal?.length || 0), 0);
    const realizedPnlUsd = allDecisions.reduce((sum, r) => sum + (Number(r.realizedPnlUsd) || 0), 0);
    return {
      ok: true,
      ...
      opportunities: [...d.opportunities].map((o) => {
        ...
        const d2 = (d.decisions || {})[k];
        return d2 ? Object.assign({}, o, { decision: d2 }) : o;
      }).sort((a, b) => (!!a.suspicious - !!b.suspicious) || ((b.qualityScore || 0) - (a.qualityScore || 0)) || ((b.spreadPct || 0) - (a.spreadPct || 0))).slice(0, 200),
      topWallets: wallets.filter((w) => !w.ignored && !w.likelyContract && !w.passiveAccount).sort((a, b) => b.score - a.score).slice(0, 8),
  ...
  api['/api/transfers'] = async (ctx) => {
    ...
    const maxTransfers = store.settings()?.scan?.maxTransfers || 8000;
    return { ok: true, total: res.total, items: res.items, maxTransfers };
  };
  ...
  api['/api/token/check'] = async (ctx) => {
    ...
    if (r.best && r.best.spreadPct > 0 && r.best.verdict !== 'fake' && !r.best.isSymbolCollision && !r.best.collisionRisk) {
      store.addOpportunities([r.best]);
      events.broadcast('opportunities', { count: 1, items: [r.best] });
      try {
        notifier.notifyOpportunities([r.best], { settings: store.settings(), force: true });
      } catch (err) {
        console.error('[Notifier] Failed to notify opportunity from token check:', err.message);
      }
    }
  ...
  api['/api/opportunity/live'] = async (ctx) => {
    const params = Object.assign({}, ctx.query || {}, ctx.body || {});
    const {
      symbol,
      buyChain,
      buyAddress,
    ...
    if (!buyChain || !sellChain) {
      return { ok: false, error: '缺少买卖链参数 buyChain / sellChain' };
    }

    if (traffic.isPaused()) return { ok: false, error: '全部外部请求已暂停，请先恢复请求' };
    const settings = store.settings();
    const forceRefresh = force === true || force === 'true' || force === '1';

    // 步骤 1：先实时获取买卖两端 DEX 现货最新报价与池子深度（优先使用发现机会时的准确交易对池地址）
    const [buyQuote, sellQuote] = await Promise.all([
      buyAddress ? prices.quote(buyChain, buyAddress, settings, buyPairAddress, forceRefresh) : null,
      sellAddress ? prices.quote(sellChain, sellAddress, settings, sellPairAddress, forceRefresh) : null,
    ]);

    // 步骤 2：并发获取买卖两端流动性池手续费 (Pool Swap Fee) 与代币合约安全性
    const resolvedBuyPair = buyPairAddress || buyQuote?.pairAddress;
    const resolvedSellPair = sellPairAddress || sellQuote?.pairAddress;
    const [buySec, sellSec] = await Promise.all([
      buyAddress ? securityChecker.checkTokenSecurity(buyChain, buyAddress, settings, resolvedBuyPair) : null,
      sellAddress ? securityChecker.checkTokenSecurity(sellChain, sellAddress, settings, resolvedSellPair) : null,
    ]);

    const liveBuyPrice = buyQuote?.priceUsd > 0 ? buyQuote.priceUsd : 0;
    const liveSellPrice = sellQuote?.priceUsd > 0 ? sellQuote.priceUsd : 0;
    const liveSpreadPct = liveBuyPrice > 0 ? Number((((liveSellPrice - liveBuyPrice) / liveBuyPrice) * 100).toFixed(3)) : 0;
    const principal = Number(amountUsd) || 1000;
    const tokenAmount = liveBuyPrice > 0 ? Number((principal / liveBuyPrice).toFixed(4)) : 0;

    // 提取两端交易池手续费与代币税率
    const buyPoolFee = typeof buySec?.poolFee === 'number' ? buySec.poolFee : 0.003;
    const sellPoolFee = typeof sellSec?.poolFee === 'number' ? sellSec.poolFee : 0.003;
    const buyTax = typeof buySec?.buyTax === 'number' ? buySec.buyTax : 0;
    const sellTax = typeof sellSec?.sellTax === 'number' ? sellSec.sellTax : 0;
    const isTrapPool = Boolean(buySec?.isTrapPool || sellSec?.isTrapPool);

    // 步骤 3：在取得实时代币价格与搬运数量后，再进行链上跨链通道询价（传递实时代币单价与真实搬运数量）
    const liveBridge = await quoter.getLiveQuote({
      buyChain,
      sellChain,
      buyAddress,
      sellAddress,
      tokenPrice: liveBuyPrice,
      tokenAmount,
      amountUsd: principal,
      force: forceRefresh,
    }).catch((e) => {
      console.warn('[quoter] liveQuote error:', e.message);
      return null;
    });
    ...
    let statusMessage = '无法获取两端 DEX 实时行情';
    let simulation = null;

    if (liveBuyPrice > 0 && liveSellPrice > 0) {
      live = {
        buyPrice: liveBuyPrice,
        sellPrice: liveSellPrice,
        spreadPct: liveSpreadPct,
        tokenAmount,
        buyDex: buyQuote?.dex,
        sellDex: sellQuote?.dex,
        buyPairAddress: buyQuote?.pairAddress,
        sellPairAddress: sellQuote?.pairAddress,
        buyPoolFee,
        sellPoolFee,
        buyPoolType: buySec?.poolType || buyQuote?.dex,
        sellPoolType: sellSec?.poolType || sellQuote?.dex,
        buyTax,
        sellTax,
        isTrapPool,
        buyLiquidityUsd: buyQuote?.liquidityUsd,
        sellLiquidityUsd: sellQuote?.liquidityUsd,
        buyVolume24h: buyQuote?.volume24h,
        sellVolume24h: sellQuote?.volume24h,
        buyPairUrl: buyQuote?.pairUrl,
        sellPairUrl: sellQuote?.pairUrl,
        updatedAt: Date.now(),
      };

      const snapBuy = Number(snapshotBuyPrice) || liveBuyPrice;
      const snapSell = Number(snapshotSellPrice) || liveSellPrice;
      const snapSpread = Number(snapshotSpreadPct) || liveSpreadPct;

      drift = {
        buyPriceDeltaPct: snapBuy > 0 ? Number((((liveBuyPrice - snapBuy) / snapBuy) * 100).toFixed(2)) : 0,
        sellPriceDeltaPct: snapSell > 0 ? Number((((liveSellPrice - snapSell) / snapSell) * 100).toFixed(2)) : 0,
        spreadDeltaPct: Number((liveSpreadPct - snapSpread).toFixed(2)),
      };

      // 净盈亏动态测算 (全面扣除跨链通道成本 + 买卖双端 DEX 池子手续费与代币交易税)
      const grossProfitUsd = Number(((liveSpreadPct / 100) * principal).toFixed(2));
      const bridgeCostUsd = liveBridge?.totalFeeUsd != null ? liveBridge.totalFeeUsd : 4.5;

      const buyFrictionUsd = principal * (buyPoolFee + buyTax);
      const grossRevenueUsd = principal * (1 + liveSpreadPct / 100);
      const sellFrictionUsd = grossRevenueUsd * (sellPoolFee + sellTax);
      const dexFrictionCostUsd = Number((buyFrictionUsd + sellFrictionUsd).toFixed(2));

      const totalCostUsd = Number((bridgeCostUsd + dexFrictionCostUsd).toFixed(2));
      const netProfitUsd = Number((grossProfitUsd - totalCostUsd).toFixed(2));
      const netYieldPct = Number(((netProfitUsd / principal) * 100).toFixed(2));

      simulation = {
        evidence: 'display_price_model',
        executable: false,
        principalUsd: principal,
        grossProfitUsd,
        bridgeCostUsd,
        dexFrictionCostUsd,
        buyFrictionUsd: Number(buyFrictionUsd.toFixed(2)),
        sellFrictionUsd: Number(sellFrictionUsd.toFixed(2)),
        totalCostUsd,
        netProfitUsd,
        netYieldPct,
        isTrapPool,
      ...
      const isExtremeSpread = liveSpreadPct > 100;
      const buyDivergence = snapBuy > 0 ? Math.max(liveBuyPrice / snapBuy, snapBuy / liveBuyPrice) : 1;
      const sellDivergence = snapSell > 0 ? Math.max(liveSellPrice / snapSell, snapSell / liveSellPrice) : 1;
      const isExtremeDivergence = (snapBuy > 0 && buyDivergence > 3.0) || (snapSell > 0 && sellDivergence > 3.0);

      if (isTrapPool) {
        status = 'TRAP_POOL';
        const maxFee = Math.max(buyPoolFee, sellPoolFee);
        statusMessage = `高费率陷阱池警报：检测到交易池 (${buyPoolFee >= 0.05 ? (buyQuote?.dex || '买入池') : (sellQuote?.dex || '卖出池')}) Swap手续费高达 ${(maxFee * 100).toFixed(1)}%！DEX Screener 默认隐藏此费率，往返摩擦将直接吞噬全部利润！`;
      } else if (isExtremeSpread || isExtremeDivergence) {
        status = 'ANOMALY_SPREAD';
        statusMessage = isExtremeSpread
          ? `极端异常价差 (+${liveSpreadPct}%)：价差严重超出跨链常理 (>100%)，疑似同名不同资产冲突、坏账池或虚假流动性，已触发保护熔断！`
          : `实时报价剧烈偏离：当前价格与快照偏离超 3.0×，行情剧烈异动，已触发保护熔断！`;
        simulation.netProfitUsd = 0;
        simulation.netYieldPct = 0;
        simulation.isAnomaly = true;
      } else if (liveSpreadPct <= 0) {
        status = 'INVERTED';
        statusMessage = `价差已抹平/倒挂 (${liveSpreadPct}%)：买价现高于或等于卖价，切勿执行！`;
      } else if (netProfitUsd <= 0) {
        status = 'NARROWED';
        statusMessage = `净利润收窄/为负 (${liveSpreadPct}%)：毛利 $${grossProfitUsd} 不足以覆盖池子手续费 ($${dexFrictionCostUsd}) 与跨链费用 ($${bridgeCostUsd})`;
      } else if ((buyQuote?.liquidityUsd || 0) < 1000 || (sellQuote?.liquidityUsd || 0) < 1000) {
        status = 'LIQUIDITY_DROP';
        statusMessage = `流动性不足 (买端 $${Math.round(buyQuote?.liquidityUsd || 0)} / 卖端 $${Math.round(sellQuote?.liquidityUsd || 0)})`;
      } else {
        status = 'UNAVAILABLE';
        statusMessage = `仅有展示价差 +${liveSpreadPct}%；请使用闭环验证检查实际成交数量、四段费用和回款。此估算不证明可以盈利。`;
      }
    }
    ...
    return {
      ...
      sellAddress,
      snapshot: {
        buyPrice: Number(snapshotBuyPrice) || null,
        sellPrice: Number(snapshotSellPrice) || null,
        spreadPct: Number(snapshotSpreadPct) || null,
        ts: snapshotTs || null,
      },
      live,
      drift,
      bridge: liveBridge ? {
        ok: liveBridge.ok,
        bridgeName: liveBridge.bridgeName,
        bridgeUrl: liveBridge.bridgeUrl,
        hasApiKey: liveBridge.hasApiKey,
        isDirectTokenRoute: liveBridge.isDirectTokenRoute,
        source: liveBridge.source,
        bridgeFeeUsd: liveBridge.bridgeFeeUsd,
        gasFeeUsd: liveBridge.gasUsd,
        gasUsd: liveBridge.gasUsd,
        totalCostUsd: liveBridge.totalFeeUsd,
        totalFeeUsd: liveBridge.totalFeeUsd,
        etaSeconds: liveBridge.etaSeconds,
        isLiveQuote: liveBridge.isLiveQuote,
        ttlSeconds: liveBridge.ttlSeconds || 60,
        updatedAt: liveBridge.updatedAt || Date.now(),
        expiresAt: liveBridge.expiresAt || (Date.now() + 60000),
        details: liveBridge.details,
        tokenAmount,
        live,
        drift,
        status,
  ...
  api['/api/database/reset'] = async (ctx) => {
    if (ctx.req.method !== 'POST') {
      return { ok: false, error: '仅支持 POST 请求' };
    }
    const { confirmPhrase, keepSettings } = ctx.body || {};
    if (String(confirmPhrase || '').trim() !== 'RESET') {
      return {
        ok: false,
        error: '安全确认码不匹配。必须输入 "RESET" 才能重置数据库。',
      };
    }
    try {
      const res = store.resetStore({ keepSettings: keepSettings !== false });
      return {
        ok: true,
        message: '数据库已成功重置并清空所有业务数据',
        ...res,
      };
    } catch (e) {
      return { ok: false, error: e.message };
  ...
  api['/api/storage/reset'] = api['/api/database/reset'];
  ...
  // All views reuse the same in-flight live check and its short display cache.
  const liveChecks = new Map();
  const liveHandler = api['/api/opportunity/live'];
  api['/api/opportunity/live'] = async ctx => {
    if (traffic.isPaused()) return { ok: false, error: '全部外部请求已暂停，请先恢复请求' };
    const params = { ...ctx.query, ...ctx.body };
    const force = [true, 'true', '1'].includes(params.force);
    delete params.force; delete params._t;
    const key = JSON.stringify(Object.keys(params).sort().map(k => [k, params[k]]));
    const entry = liveChecks.get(key);
    if (entry && (entry.pending || Date.now() - entry.finishedAt < (force ? 2000 : 10000))) return structuredClone(await entry.promise);
    if (liveChecks.size >= 200) for (const [id, value] of liveChecks) { if (!value.pending) { liveChecks.delete(id); break; } }
    if (liveChecks.size >= 200 && !entry) return { ok: false, error: '实时验价任务过多，请稍后重试' };
    const fresh = { pending: true, finishedAt: 0 };
    fresh.promise = liveHandler(ctx).finally(() => { fresh.pending = false; fresh.finishedAt = Date.now(); });
    liveChecks.set(key, fresh);
    return structuredClone(await fresh.promise);
  };

  api['/api/quote/live'] = async (ctx) => {
    try {
      const oppRes = await api['/api/opportunity/live'](ctx);
      if (oppRes.ok) {
        if (oppRes.bridge) {
          return Object.assign({ ok: true }, oppRes.bridge, {
            simulation: oppRes.simulation,
            status: oppRes.status,
            statusMessage: oppRes.statusMessage,
          });
        }
        return {
          ok: true,
          ...
          status: oppRes.status,
          statusMessage: oppRes.statusMessage,
          checkedAt: oppRes.checkedAt,
          details: {
            note: '跨链通道未直连报价，已获取两端DEX现价与流动性池手续费',
          },
        };
      }
    } catch (e) {
      console.warn('[quote/live] opportunity/live error, fallback to direct bridge quote:', e.message);
    }
  ...
  api['/api/security/check'] = async (ctx) => {
    const { chain, address, pairAddress, pair } = ctx.query || {};
    if (!chain || !address) {
      return { ok: false, error: 'Missing chain or address' };
    }
    try {
      const res = await securityChecker.checkTokenSecurity(chain, address, store.settings(), pairAddress || pair);
  ...
  api['/api/decisions'] = async (ctx) => {
    const d = store.raw();
    const dec = d.decisions || {};
    const byStatus = { todo: 0, watching: 0, executed: 0, closed: 0, dropped: 0, unmarked: 0, marked: 0 };
    ...
    for (const o of [...d.opportunities].sort((a, b) => (b.ts || '').localeCompare(a.ts || ''))) {
      ...
      seenKeys.add(k);
      const rec = dec[k];
      const marked = isActivelyMarked(rec);
      const status = marked ? rec.status : 'todo';
      if (!marked) {
        byStatus.unmarked++;
        byStatus.todo++;
      } else {
    ...
    for (const [k, rec] of Object.entries(dec)) {
      ...
      const marked = isActivelyMarked(rec);
      if (!marked) continue;
      const status = rec.status || 'watching';
      byStatus.marked++;
      if (byStatus[status] != null) byStatus[status]++;
      rows.push({
        key: k,
    ...
    const byMarked = (r) => (isActivelyMarked(r.decision) ? 0 : 1);
    rows.sort((a, b) => byMarked(a) - byMarked(b) || String(b.ts || '').localeCompare(String(a.ts || '')));
    const filter = ctx.query.status;
    let out = rows;
    if (filter && filter !== 'all') {
      if (filter === 'unmarked') out = rows.filter((r) => !isActivelyMarked(r.decision));
      else if (filter === 'marked') out = rows.filter((r) => isActivelyMarked(r.decision));
      else out = rows.filter((r) => r.status === filter && isActivelyMarked(r.decision));
    }
    // 全局已实现盈亏：统计全量决策账本，不因机会退市而丢失
    const pnl = Object.values(dec).reduce((s, r) => s + (Number(r.realizedPnlUsd) || 0), 0);
  ...
  api['/api/decisions/status'] = async (ctx) => {
    ...
    const existing = store.raw().decisions?.[k];
    if (b.status === 'todo' && (!b.note && !existing?.note) && (!existing?.journal?.length) && (!existing?.realizedPnlUsd)) {
      store.removeDecision(k);
  ...
  api['/api/settings'] = async (ctx) => {
    ...
    if (req.method === 'POST') {
      ...
      d.settings = { ...previous, ...patch };
      for (const name of ['keys', 'sources', 'scan', 'endpoints', 'depeg']) if (patch[name]) d.settings[name] = { ...previous[name], ...patch[name] };
      if (patch.notifications) {
        const base = store.DEFAULT_SETTINGS.notifications;
        const curr = previous.notifications || base;
  ...
  api['/api/notifications/test-tg'] = async (ctx) => {
    const { botToken, chatId } = ctx.body || {};
    const settings = store.settings();
    const result = await notifier.testTelegram({
      botToken: (botToken || '').trim() || settings.notifications?.telegram?.botToken,
      chatId: (chatId || '').trim() || settings.notifications?.telegram?.chatId,
      settings,
    });
    return result;
  };

  api['/api/notifications/get-chat-id'] = async (ctx) => {
    const { botToken } = ctx.body || {};
    const settings = store.settings();
    const result = await notifier.detectChatId({
      botToken: (botToken || '').trim() || settings.notifications?.telegram?.botToken,
      settings,
    });
    return result;
  ...
  api['/api/notifications/get-bot-info'] = async (ctx) => {
    const { botToken } = ctx.body || {};
    const settings = store.settings();
