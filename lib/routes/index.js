...
function createApiRoutes(context) {
  ...
  api['/api/opportunity/live'] = async (ctx) => {
    ...
    // 步骤 1：先实时获取买卖两端 DEX 现货最新报价与池子深度
    const [buyQuote, sellQuote] = await Promise.all([
      buyAddress ? prices.quote(buyChain, buyAddress, settings) : null,
      sellAddress ? prices.quote(sellChain, sellAddress, settings) : null,
    ]);
    ...
    if (liveBuyPrice > 0 && liveSellPrice > 0) {
      ...
      } else if (buyQuote.liquidityUsd < 1000 || sellQuote.liquidityUsd < 1000) {
        status = 'LIQUIDITY_DROP';
        statusMessage = `流动性不足 (买端 $${Math.round(buyQuote.liquidityUsd)} / 卖端 $${Math.round(sellQuote.liquidityUsd)})`;
      } else {
        status = 'ACTIVE';
