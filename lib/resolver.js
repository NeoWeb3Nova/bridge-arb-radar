...
function addEntry(idx, sym, chain, entry) {
  const addr = addressKey(entry.address);
...
async function coingeckoLookup(symbol, settings) {
  ...
  for (const [pid, addr] of Object.entries(platforms)) {
    ...
    const e = { chain: ck, address: addressKey(addr), name: cRes.json.name || null, decimals: null, source: 'coingecko' };
    entries.push(e);
...
function resolveSymbol(symbol, opts = {}) {
  ...
  for (const tok of Object.values(d.tokens || {})) {
    ...
    push(tok.chain, { address: addressKey(tok.address), name: null, decimals: null, source: 'bridge' });
  }
  ...
  const cleanName = (s) => {
    ...
    return String(s)
      .toLowerCase()
      .replace(/\s*\([^)]*\)/g, '') // 移除链标识后缀如 (PoS), (Arbitrum), (Wormhole)
      .replace(/\b(bridged|portal|wormhole|wrapped|token)\b/gi, '')
      .replace(/[^a-z0-9]/g, '')
      .trim();
  };
...
function identityOf(chain, address) {
  ...
  const idx = loadRegistry();
  const hit = idx.byAddress[chain]?.[addressKey(address)];
  if (!hit) return null;
  return { symbol: hit.symbol, name: hit.name, chain, address: addressKey(address) };
}
