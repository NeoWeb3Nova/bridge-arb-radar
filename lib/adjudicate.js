...
async function adjudicateAmbiguous(symbol, entries, settings) {
  ...
  for (const q of quoted) {
    ...
    if (priceFake) {
      verdict = 'fake';
      reason = isVerified
        ? `官方注册表同名但跨链价格偏离 ${ratio.toFixed(1)}×（证实为同名不同资产或坏账池）`
        : `价格偏离锚点 ${ratio.toFixed(1)}×（≥${FAKE_RATIO}×）`;
    } else if (anchorName && onChainName && !nameMatch && !onChainName.includes(anchorName) && !anchorName.includes(onChainName)) {
      verdict = 'fake';
      reason = `同名不同币 · 链上全称不匹配（「${quote.baseTokenName || '?'}」≠「${anchorName}」）`;
    } else if (isVerified) {
      verdict = 'official';
      reason = '官方注册表';
    } else if (!symMatch) {
    ...
    const qo = Object.assign({}, quote, {
      verified: isVerified && verdict !== 'fake',
      verdict,
      reason,
      explorerUrl: eu,
