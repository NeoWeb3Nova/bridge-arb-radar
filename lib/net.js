...
const defaultHttpAgent = new http.Agent({ keepAlive: true, maxSockets: 32, keepAliveMsecs: 15000 });
const defaultHttpsAgent = new https.Agent({ keepAlive: true, maxSockets: 32, keepAliveMsecs: 15000 });
 ...
 */
async function sendRequest(url, opts = {}) {
  ...
  try {
    ...
    if (proxy && isHttps) {
      ...
      await new Promise((res, rej) => {
        const cleanup = () => { if (timer) clearTimeout(timer); try { tunnel.destroy(); } catch {} };
        tlsSock.once('secureConnect', () => { if (timer) clearTimeout(timer); res(); });
        tlsSock.once('error', (err) => { cleanup(); rej(err); });
        timer = setTimeout(() => { cleanup(); rej(new Error('tls timeout')); }, timeout);
        timer.unref?.();
      });
      ...
      payload = await send(http, { agent: defaultHttpAgent, host: proxy.host, port: proxy.port, path: url, method, headers: Object.assign({ Host: target.host }, headers), timeout }, body);
    } else {
      payload = await send(isHttps ? https : http, { agent: isHttps ? defaultHttpsAgent : defaultHttpAgent, host: target.hostname, port: target.port || (isHttps ? 443 : 80), path: target.pathname + target.search, method, headers, timeout }, body);
    }
    let json = null;
    try { json = JSON.parse(payload.text); } catch { json = null; }
    return { ok: payload.status >= 200 && payload.status < 300, status: payload.status, retryAfter: payload.retryAfter, rateLimitReset: payload.rateLimitReset, rateLimitRemaining: payload.rateLimitRemaining, text: payload.text, json };
  } catch (err) {
...
function send(mod, options, body) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (finished) return;
      finished = true;
      try { req.destroy(new Error('request overall timeout')); } catch {}
      reject(new Error('request overall timeout'));
    }, timeoutMs + 2000);
    timer.unref?.();
    ...
    const req = mod.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        finished = true;
        clearTimeout(timer);
        resolve({ status: res.statusCode, retryAfter: res.headers['retry-after'], rateLimitReset: res.headers['ratelimit-reset'], rateLimitRemaining: res.headers['ratelimit-remaining'], text: Buffer.concat(chunks).toString('utf8') });
      });
      res.on('error', (err) => {
        if (finished) return;
        finished = true;
...
const { createGovernor, QUOTE_LIMIT, KEY_QUOTE_LIMIT } = require('./request-governor');
const governor = createGovernor({
  file: require('path').join(__dirname, '..', 'data', 'request-budget.json'),
  transport: sendRequest,
  quoteLimit: () => {
    try {
      const store = require('./store');
      const s = store.settings();
      if (Number.isFinite(s?.scan?.quoteLimit) && s.scan.quoteLimit > 0) return s.scan.quoteLimit;
      const hasKey = Boolean(String(s?.keys?.lifi || process.env.LIFI_API_KEY || '').trim());
      if (hasKey) {
        const rpm = Number(s?.keys?.lifiRpm) || 100;
        return rpm * 120; // LI.FI official 2-hour rolling window: RPM * 120 (12,000 for 100 RPM)
      }
    } catch {}
    return QUOTE_LIMIT;
  },
});
function request(url, opts) { return governor.request(url, opts); }
module.exports = { request, getJSON, UA, governor };
