...
async function request(url, opts = {}) {
  ...
  try {
    ...
    return { ok: payload.status >= 200 && payload.status < 300, status: payload.status, retryAfter: payload.retryAfter, text: payload.text, json };
  } catch (err) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = mod.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        finished = true;
        clearTimeout(timer);
        resolve({ status: res.statusCode, retryAfter: res.headers['retry-after'], text: Buffer.concat(chunks).toString('utf8') });
      });
      res.on('error', (err) => {
        if (finished) return;
        finished = true;
