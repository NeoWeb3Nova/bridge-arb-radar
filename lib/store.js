...
function touchWallet(key) { if (key) dirty.wallets.add(addressKey(key)); }
...
function deepMergeDefaults(raw) {
  ...
  s.stablecoins = Array.isArray(raw?.stablecoins) && raw.stablecoins.length > 0
    ? raw.stablecoins
    : DEFAULT_SETTINGS.stablecoins;
  s.depeg = Object.assign({}, DEFAULT_SETTINGS.depeg, raw?.depeg || {});
  return s;
}
...
function upsertToken(chain, address, patch = {}) {
  ...
  const t = d.tokens[key] || {
    chain, address: addressKey(address),
    firstSeen: new Date().toISOString(),
    bridges: 0,
    wallets: {},
    routes: {},
...
function addOpportunities(list) {
  ...
  let added = 0;
  for (const o of list) {
    const idx = d.opportunities.findIndex((x) => x.symbol === o.symbol && x.buyChain === o.buyChain && x.sellChain === o.sellChain && addressKey(x.buyAddress) === addressKey(o.buyAddress) && addressKey(x.sellAddress) === addressKey(o.sellAddress));
    if (idx >= 0) {
      d.opportunities[idx] = Object.assign(d.opportunities[idx], o, { firstSeen: d.opportunities[idx].firstSeen || d.opportunities[idx].ts, ts: new Date().toISOString(), hits: (d.opportunities[idx].hits || 0) + 1 });
    } else {
...
function removeOpportunities(predicate) {
  if (typeof predicate !== 'function') return 0;
  const d = load();
  const initial = d.opportunities.length;
  d.opportunities = d.opportunities.filter(o => !predicate(o));
  if (d.opportunities.length !== initial) {
    dirty.opportunities = true;
    save();
    flush();
  }
  return initial - d.opportunities.length;
}
...
/**
 * 重置数据仓库：清空 SQLite 存储并重置内存对象。
 * 支持保持当前配置项（keepSettings = true）。
 */
function resetStore({ keepSettings = true } = {}) {
  if (saveTimer) {
  ...
  const currentMem = load();
  const prevTransfers = currentMem.transfers?.length || 0;
  const prevWallets = Object.keys(currentMem.wallets || {}).length;
  const prevTokens = Object.keys(currentMem.tokens || {}).length;
  const prevOpps = currentMem.opportunities?.length || 0;
  const prevDecisions = Object.keys(currentMem.decisions || {}).length;

  const preservedSettings = keepSettings && currentMem.settings
    ? JSON.parse(JSON.stringify(currentMem.settings))
    : deepMergeDefaults({});

  const dbRes = db.resetDatabase({
    keepSettings,
    defaultSettings: preservedSettings,
  });
  ...
  return {
    ok: true,
    backupFile: dbRes.backupFile,
    keptSettings: !!keepSettings,
    wiped: {
      transfers: prevTransfers,
      wallets: prevWallets,
      tokens: prevTokens,
      opportunities: prevOpps,
...
module.exports = {
  ...
  addTransfers, upsertWallet, upsertToken, addOpportunities, removeOpportunities, tokenKey, DEFAULT_SETTINGS,
  ...
  backupNow, pruneBackups, storageStatus, compact, resetStore,
};
